...
class Cx extends Taglib
{
    ...
    public function tagPresent($tag, $content)
    {
        $name     = $tag['name'];
        $name     = $this->autoBuildVar($name);
        // 不能对函数调用使用 isset，使用三元表达式检查
        $parseStr = '<?php if((' . $name . ' !== \'\' && ' . $name . ' !== null) || (is_array(' . $name . ') && !empty(' . $name . '))): ?>' . $content . '<?php endif; ?>';
        return $parseStr;
    }
    ...
    public function tagLoad($tag, $content)
    {
        ...
        if (isset($tag['value'])) {
            ...
            $name = $this->autoBuildVar($name);
            // 不能对函数调用使用 isset，使用三元表达式检查
            $parseStr .= '<?php if((' . $name . ' !== \'\' && ' . $name . ' !== null) || (is_array(' . $name . ') && !empty(' . $name . '))): ?>';
            $endStr = '<?php endif; ?>';
        }
